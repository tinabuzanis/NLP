...
def main():
    ...
    source_tokenizer = transformers.PreTrainedTokenizerFast.from_pretrained(pretrained_model_name_or_path=src_tokenizer_path)
    target_tokenizer = transformers.PreTrainedTokenizerFast.from_pretrained(pretrained_model_name_or_path=tgt_tokenizer_path)
    # YOUR CODE ENDS HERE
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for batch in train_dataloader:
            ...
            if global_step % args.eval_every_steps == 0 or global_step == args.max_train_steps:
                eval_results, decoded_preds, decoded_labels, eval_input_ids = evaluate_model(
                    model=model,
                ...
                logger.info(f"Input sentence: {source_tokenizer.decode(eval_input_ids[random_index], skip_special_tokens=True)}")
