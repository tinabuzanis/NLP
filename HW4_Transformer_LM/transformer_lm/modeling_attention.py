...
class SelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # q -> [batch_size, q_len, hidden]
        # k -> [batch_size, k_len, hidden]
        # v -> [batch_size, v_len, hidden]
        qkt = (torch.matmul(q, k.transpose(-2, -1))) / self.scale
        # qkt -> [batch_size, q_len, k_len]
        attention = torch.bmm(F.softmax(qkt, dim=2), v)
        # attention -> [batch_size, q_len, hidden]
...
class MultiHeadSelfAttention(nn.Module):
    ...
    def forward(self, x, return_attention=False):
        ...
        q = self.q(x).view(bs, -1, self.num_heads,
                           self.head_size).permute(0, 2, 1, 3)
        # q -> [batch_size, num_heads, q_len, head_size]
        k = self.k(x).view(x.size(0), -1, self.num_heads,
                           self.head_size).permute(0, 2, 1, 3)
        # k -> [batch_size, num_heads, k_len, head_size]
        v = self.v(x).view(x.size(0), -1, self.num_heads,
                           self.head_size).permute(0, 2, 1, 3)
        # v -> [batch_size, num_heads, v_len, head_size]
        ...
        if self.causal:
            ...
            causal_mask = torch.tril(
                    torch.ones((x.size(1), x.size(1)))).expand(
                    x.size(0), 1, x.size(1), x.size(1)
                    ).to(scores.device)
            # causal_mask -> [batch_size, 1, seq, seq]
    ...
    # return mask
        ...
        # probs -> [batch_size, num_heads, q_len, k_len]
        probs = self.dropout(probs)
        # probs -> [batch_size, num_heads, q_len, k_len]
        att = torch.matmul(probs, v).permute(0, 2, 1, 3)
        # att -> [batch_size, q_len, num_heads, head_size]
        probs = probs.reshape(bs * self.num_heads, seq, -1)
        # probs -> [batch_size * num_heads, seq, seq]
        att = self.mix(att.reshape(*att.shape[:2], -1))
        # att -> [batch_size, seq, hidden]
